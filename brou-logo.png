...
def is_anulado_pair(df_sap):
    """
    Devuelve set de índices SAP que forman par anulado → gris.

    FIX:
    Antes tomaba el primer número largo del comentario y buscaba con contains().
    Eso podía agarrar una línea equivocada.
    Ahora:
      1) extrae el número de pago referenciado al final o luego de "pago recibido"
      2) compara contra el Nº Doc normalizado de forma exacta
      3) solo pinta gris la línea de cancelación + su documento original exacto
    """
    cancel_pat = re.compile(
        r'(cancelar|anular).*entrada\s+para\s+n[uú]mero\s+de\s+pago',
        re.IGNORECASE
    )
    gris_idx = set()
    ...
    for i, row in df_sap.iterrows():
        com = str(row['comentarios'])

        if not cancel_pat.search(com):
            continue

        # Prioridad 1: número al final del comentario
        m = re.search(r'(\d{4,8})\s*$', com.strip())

        # Prioridad 2: número después de "pago recibido" o "pago"
        if not m:
            m = re.search(r'pago(?:\s+recibido)?\s+(\d{4,8})', com, re.IGNORECASE)

        # Prioridad 3: último número largo del comentario
        if not m:
            nums = re.findall(r'\d{4,8}', com)
            ref = nums[-1] if nums else None
        else:
            ref = m.group(1)
        ...
        # Match exacto contra Nº Doc normalizado.
