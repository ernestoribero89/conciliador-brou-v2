...
def detectar_anulados_por_importe_sap(sap):
    """
    Detecta pares anulados en SAP por importe opuesto.

    Uso para UYU:
    - Hay casos donde el texto no dice claramente "Anulación" o "Cancelar".
    - Si dos movimientos SAP se cancelan matemáticamente (+X y -X),
      con fecha cercana y comentario compatible, ambos deben quedar GRIS
      y no participar del matching contra banco.

    Criterios conservadores:
    - importe opuesto dentro de tolerancia
    - fecha ±3 días
    - evita Wiz para no interferir con pagos agrupados
    - evita diferencias de cambio
    """
    anulados_extra = set()
    ...
    def compatible_text(a, b):
        ta = clean_text(a)
        tb = clean_text(b)

        if 'anul' in ta or 'anul' in tb or 'cancel' in ta or 'cancel' in tb:
            return True

        if ta == tb:
            return True

        comunes = ['uyu', 'eur', 'usd', 'ajuste', 'asiento', 'trf', 'transferencia']
        if any(k in ta for k in comunes) and any(k in tb for k in comunes):
            return True

        if (ta == '' or tb == '') and not any(x in (ta + tb) for x in ['wiz', 'proveedor', 'pagos efectuados']):
            return True

        return False

    for i, r1 in enumerate(sap):
        if r1.get('an') or r1.get('m'):
            continue
        if r1.get('wiz') is not None and not nan_safe(r1.get('wiz')):
            continue
        if r1.get('imp') is None:
            continue

        for j in range(i + 1, len(sap)):
            r2 = sap[j]

            if r2.get('an') or r2.get('m'):
                continue
            if r2.get('wiz') is not None and not nan_safe(r2.get('wiz')):
                continue
            if r2.get('imp') is None:
                continue

            if abs((r1['f'] - r2['f']).days) > DIAS:
                continue

            if abs(round(r1['imp'] + r2['imp'], 2)) > TOLERANCIA:
                continue

            if not compatible_text(r1.get('com', ''), r2.get('com', '')):
                continue
