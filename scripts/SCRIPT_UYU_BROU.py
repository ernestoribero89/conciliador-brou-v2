...
def detectar_anulados_por_importe_sap(sap):
    """
    Detecta anulados SAP en UYU sin comerse la reemisión correcta.

    Caso objetivo:
      SAP original     -3.000  → gris
      SAP anulación    +3.000  → gris
      SAP reemisión    -3.000  → queda libre para matchear contra banco (verde)

    Reglas:
    1) Si hay comentario con referencia ("Anulación - 265201", "Cancelar entrada..."),
       usa esa referencia exacta y exige importe opuesto.
    2) Si no hay referencia, NO pinta todos los importes opuestos.
       Solo arma un par:
         - toma una línea con señal de anulación/cancelación si existe,
         - si no existe, toma el movimiento positivo como probable reversa,
         - lo empareja con el opuesto más cercano, priorizando uno anterior.
    3) Nunca marca más de 2 líneas para una misma anulación.
    4) Evita Wiz y diferencias de cambio.
    """
    anulados_extra = set()
    ...
    def has_cancel_text(v):
        t = clean_text(v)
        return 'anul' in t or 'cancel' in t
    ...
    def is_available(r):
        if r.get('an') or r.get('m'):
            return False
        if r.get('wiz') is not None and not nan_safe(r.get('wiz')):
            return False
        if r.get('imp') is None:
            return False
        return True
    ...
    def extract_ref(com):
        ...
        # "Anulación - 265201" o similar
        m = re.search(r'[-–]\s*(\d{4,8})\s*$', com)
        if m:
            return m.group(1)

        # "pago recibido 265201" / "pago 265201"
        m = re.search(r'pago(?:\s+recibido)?\s+(\d{4,8})', com, re.IGNORECASE)
        if m:
            return m.group(1)

        # último número largo solo si el texto habla de anulación/cancelación
        if has_cancel_text(com):
    ...
    # 1) Referencia explícita: exacta y segura.
    for i, r in enumerate(sap):
        if not is_available(r):
            continue

        ref = extract_ref(r.get('com', ''))
        if not ref:
            continue

        imp_i = imp_ok(r.get('imp'))
        if imp_i is None:
            continue

        for j, r2 in enumerate(sap):
            if i == j or not is_available(r2):
                continue

            if only_digits(r2.get('doc', '')) != str(ref):
                continue

            imp_j = imp_ok(r2.get('imp'))
            if imp_j is None:
                continue

            if abs(round(imp_i + imp_j, 2)) <= TOLERANCIA:
                anulados_extra.add(i)
                anulados_extra.add(j)
                break

    # 2) Fallback conservador: formar pares, no grupos.
    used = set(anulados_extra)

    # Priorizo filas que dicen anulación/cancelación; si no, positivos como reversa.
    indices = [
        i for i, r in enumerate(sap)
        if i not in used
        and is_available(r)
        and (has_cancel_text(r.get('com', '')) or (imp_ok(r.get('imp')) or 0) > 0)
    ]

    indices.sort(key=lambda i: (0 if has_cancel_text(sap[i].get('com', '')) else 1, sap[i]['f']))
    ...
    for i in indices:
        ...
        for j, r2 in enumerate(sap):
            ...
            dias = abs((r['f'] - r2['f']).days)
            if dias > DIAS:
                continue

            # Evitar proveedores/pagos Wiz disfrazados.
            txt = clean_text(r.get('com', '')) + " " + clean_text(r2.get('com', ''))
            if 'wiz' in txt or 'pagos efectuados' in txt or 'proveedor' in txt:
                continue

            # Preferir opuesto anterior a la anulación/reversa; si no, el más cercano.
            anterior = 0 if r2['f'] <= r['f'] else 1
            distancia_fila = abs(j - i)
            candidatos.append((anterior, dias, distancia_fila, j))
...
# ── Anulados SAP adicionales por referencia/importe opuesto ───────────────────
# Marca solo el par anulado y deja libre la reemisión para matchear banco.
