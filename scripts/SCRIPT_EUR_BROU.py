...
def is_anulado_pair(df_sap):
    """Devuelve set de índices SAP que forman pares anulado/reemisión → gris."""
    ...
    for i, row in df_sap.iterrows():
        ...
        if cancel_pat.search(com):
            # buscar número de pago referenciado
            m = re.search(r'(\d{4,})', com)
            if m:
                doc_refs.setdefault(m.group(1), []).append(i)
    for doc_num, idxs in doc_refs.items():
        # buscar el original (mismo doc_num en columna nro_doc)
        orig = df_sap[df_sap['nro_doc'].astype(str).str.contains(doc_num, na=False)].index.tolist()
        for idx in orig + idxs:
            gris_idx.add(idx)
    return gris_idx
